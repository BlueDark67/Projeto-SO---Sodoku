...
void str_echo(int sockfd, Jogo jogos[], int numJogos, DadosPartilhados *dados, int maxLinha, int timeoutCliente)
{
    ...
    printf("[LOBBY] ✅ Cliente %d/%d aceite (%d no lobby)\n", total, 10, lobby_size);
    ...
    printf("[LOBBY] Cliente pediu jogo. A aguardar disparo...\n");
    ...
    dados->numClientesLobby--;  // Saiu do lobby, entrou no jogo
    sem_post(&dados->mutex);
    ...
    /* ====================
     * FASE 5: AGUARDAR SOLUÇÃO
     * ==================== */
    
    n = readn(sockfd, (char *)&msg_recebida, sizeof(MensagemSudoku));
    
    if (n == 0) {
        printf("[INFO] Cliente desconectou\n");
        goto cleanup_e_sair;
    }
